...
export default function Files() {
  ...
  const uploadURL = `${process.env.REACT_APP_API_SERVER_URL}/files/upload`
  ...
  useEffect(() => {
    ...
    const getFiles = async () => {
      ...
      const { data, error } = await getProtectedResource("/api/files", "GET", accessToken);
      ...
      if (error) {
        setFiles(JSON.stringify(error, null, 2));
      }
    };
              ...
              <div className="col-md-12 col-xl-10">
                <div className="card text-bg-light">
                  <div className="card-header p-3">
                    <span style={{ float: "right", margin: "0px" }}><LogoutButton /></span>
